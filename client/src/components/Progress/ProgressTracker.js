...
const ProgressTracker = ({ user }) => {
  ...
  const getTopicTranslation = (topicName) => {
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 sm:py-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6 mb-6 sm:mb-8">
        ...
        <div className="bg-white rounded-xl shadow-lg p-6 hover:shadow-xl transition-shadow">
          ...
          <p className="text-3xl font-bold text-gray-900">{dashboardData.totalTimeSpent.toFixed(1)} {t('progress.minutes')}</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
        ...
        <div className="lg:col-span-2 bg-white rounded-xl shadow-lg p-4 sm:p-6 h-full flex flex-col overflow-hidden">
          ...
          {dashboardData.recentQuizzes.length > 0 ? (
            <div className="overflow-x-auto -mx-4 sm:mx-0 flex-1 overflow-y-auto min-h-0">
                ...
                <table className="min-w-full divide-y divide-gray-200">
                  <thead className="bg-gray-50 sticky top-0">
                    <tr>
                      ...
                      <th className="px-3 sm:px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">{t('dashboard.topic')}</th>
                    ...
                    {dashboardData.recentQuizzes.map((quiz, idx) => (
                      <tr key={idx} className="hover:bg-gray-50 transition-colors">
                        ...
                        <td className="px-3 sm:px-6 py-3 sm:py-4 whitespace-nowrap text-xs sm:text-sm">
                          <span className={`px-2 sm:px-3 py-1 rounded-full font-semibold ${quiz.score >= 70 ? 'bg-green-100 text-green-800' :
            ...
            {dashboardData.mastery.map((topic, idx) => (
              <div key={idx} className="mb-3 sm:mb-4">
                <div className="flex justify-between mb-2">
                  <span className="text-xs sm:text-sm font-medium text-gray-700 truncate pr-2">{getTopicTranslation(topic.topic) || t('forum.unknown')}</span>
                  <span className="text-xs sm:text-sm text-gray-600 flex-shrink-0">{topic.mastery.toFixed(2)}%</span>
                </div>
